...
export default withFormik({
  ...
  handleSubmit: async ({ name, email: requesterEmail, message }, { setSubmitting, resetForm, setFieldValue }) => {
    try {
      const encode = data =>
      Object.keys(data)
      .map(key => `${encodeURIComponent(key)}=${encodeURIComponent(data[key])}`)
		  .join('&');
      const params = encode({ name, requesterEmail, message});
      await fetch(`https://v0h4mokpng.execute-api.us-east-1.amazonaws.com/dev/email/send?${params}`, {
        method: 'GET',
        mode:'no-cors'
      });
